...
| C. 预载「不是现在」（2026-07-26 新需求） | ✅ **已随 v2.1.0 发布** |
| D. 后台「传输任务」+ 文件夹内逐文件清单（2026-07-26 新需求） | ✅ **已随 v2.1.0 发布** |
| E. 离线下载可选 Referer（2026-07-27 新需求） | ✅ **已随 v2.1.2 发布** |
| F. 存储保存不再全量重建索引（2026-07-28 新需求） | ✅ 代码+测试+真机实测完成，**待发版** |
...
# C. 预载「不是现在」（2026-07-26，已随 v2.1.0 发布）
...
# D. 后台「传输任务」+ 文件夹内逐文件清单（2026-07-26，已随 v2.1.0 发布）
...
# E. 离线下载可选 Referer（2026-07-27，已随 v2.1.2 发布）

> 用户：「离线下载能识别 m3u8 但没发 Referer，所以下不了 fansone 这种防盗链的站
>（不是 IP 鉴权），补上一个可选 Referer 字段就能用」。

离线下载此前一个请求头都不额外发：普通直链走 Go 的 `http.Client` 默认头，HLS 交给
ffmpeg 直拉。校验 Referer 的站点因此在**两个位置**都会被挡——先行 GET 取播放列表时，
以及 ffmpeg 拉每个分片时。只补前者等于没补。

## 改动

- `internal/server/handler_offline.go`
  - 请求体新增可选 `referer`，**本批链接共用**（一次粘的通常来自同一个站）。校验走新抽出的
    `httpURL()`：必须是 http/https + host 非空。**这一步是安全边界**——`url.Parse` 拒 ASCII
    控制字符，顺带挡住用 CRLF 往 ffmpeg `-headers` 里追加任意请求头。非法值直接 400，不建任务。
  - `offlineFetch`/`offlineFetchHLS` 的 6 个散参收进 `offlineJob{url,dstDir,name,referer}`，
    顺手去掉了「闭包取副本」那行手工拷贝（每轮一个 job 值天然独立）。
  - 直链：`req.Header.Set("Referer", …)`。查过 `net/http` 的 `refererForURL`：显式设过的
    Referer 在跟随重定向时**会被保留**（仅 https→http 降级按 RFC 丢弃），不必自己接管 CheckRedirect。
  - HLS：`-headers "Referer: …\r\n"` 挂在 `-i` 之前。**关键是它会不会传到分片请求**——
    libavformat 的 hls 解复用器只把 `headers`/`referer`/`user_agent`/`cookies` 等几项 avio
    选项透传给子请求，`headers` 正在其中。本机造了个只认 Referer 的 403 源站实测：
    播放列表 + 3 个分片共 4 次请求全部带头，产物 6 秒有画有声。
  - 报错：403 且没填 Referer 时，明说「若是防盗链站点，填写 Referer 后重试」，
    直链与分片两处各一条。**必须走 `util.Messagef`**——`Humanize` 见到「http 403」会一律
    翻成「没有权限执行此操作，可能需要重新授权」，把该说的话盖掉（防盗链 403 现在
    看不出原因，根因就在这条通用映射）。
- 前端：`api.js` 的 `offline()` 多传 `referer`；`Files.vue` 弹窗第三个可选输入
  「Referer（可选，防盗链站点填来源网址）」。提交成功后**连同 Referer 一起清空**——
  下次粘的可能是别的站，留着会把上一个站的来源发出去。

## 验证

- `internal/server/offline_referer_test.go`（新增）：
  - `TestHTTPURL` 10 例：CRLF/无 scheme/无 host/ftp/javascript 全拒。
  - `TestOfflineRefererDirect`：非法 Referer → 400 不建任务；不填 → 任务失败且错误里含
    「Referer」（锁住 Humanize 不会再把它压成「没有权限」）；填对 → 落盘字节一致。
  - `TestOfflineRefererHLS`（无 ffmpeg 则 Skip）：现场生成 3 分片样片 → 只认 Referer 的
    源站 → 产物 `防盗链影片.mp4` ffprobe 有 video+audio，且**每个分片请求都带 Referer**。
    这条是真不变量：把 `-headers` 去掉它立刻红。
- `go test ./...` 全绿；`frontend/offline-check.mjs` 18 通过 / 0 失败 / 3 跳过
  （跳过的仍是回环源站被 SSRF 拦的老限制），新增两条：Referer 输入框在位、非法值 400。
  负例故意走 Node 侧 fetch 直发——页面里发 400 会被记成控制台错误，污染文末「无控制台错误」。
- 弹窗 1400×900 与 390×844 各截一张，三个输入间距一致、移动端占位文字不截断。

---

# F. 存储保存不再全量重建索引（2026-07-28）

用户原话：「存储管理保存后没必要重建索引」。用户选定方案：**最小增量，只动受影响的盘**。

**问题**：`afterStorageChange` 对任何存储改动都调 `index.Rebuild()`——全量 BFS 扫遍所有网盘
（真实 OneDrive 4.3 万条约 7 分钟），完了还经 `OnComplete` 再触发一轮全库预载。改个排序、
关一下「在视频库展示」也要付这份代价。

**改法**：按"改了什么"决定动多少，且只动那一个挂载。

| 改动 | 索引动作 |
|---|---|
| 排序 / 展示开关 / 代理·加速 | 不动（`indexNeutral` 字段集） |
| 挂载路径 | `DeletePrefix(新路径)` + `RenamePrefix(旧→新)`，秒级，不重扫 |
| 驱动 / 账号 / 根目录 / 停用改启用 | 只重扫这一个盘 |
| 停用 / 删除存储 | 只删这个盘的索引行 |
| 重载、Telegram 登录成功、Google 授权回调 | 只重扫这一个盘 |

- `internal/index/index.go`：新增 `ReplaceSubtree(root)`——后台扫完整个挂载再在**单事务内**
  替换它的索引行（同 `replaceAll` 的理由：边扫边写会让这个盘在搜索/媒体库里空几分钟）。
  同一挂载已在扫时只打标记、扫完补一轮（`subtree map[string]bool`），连点保存不会并发扫两遍。
  挂载不存在/停用/Init 失败时**原样保留旧行**——扫不出东西，清空只会让一次网络抖动抹掉整个盘；
  真要摘掉由调用方显式 `DeletePrefix`。全量重建的 BFS 抽成 `walkMount` 与之共用。
  「共 N 项」改成增量写标脏（`markDirty`）、读进度时才重数一遍（`refreshCount`）——批量删除、
  目录复制是热路径，每笔扫一遍全表不值当；`DeletePrefix` 复用 `delPrefixSQL` 常量。
- `internal/server/handler_storage.go`：`afterStorageChange(c, apply)` 收 apply 回调，
  响应体带 `{scanning:bool}`；`contentCfgChanged` 只比对"决定有哪些文件"的配置字段。
- 前端：保存/重载/TG 登录的提示按 `scanning` 分岔——「已保存」vs「已保存，正在后台索引该存储」，
  不再无脑说"索引重建已触发"；AdminIndex 文案改为「日常写操作与存储改动会自动增量更新，无需重建」。
- 测试 `internal/server/storage_index_test.go`（全链路 sqlite+local+Router）：判据是一行**合成哨兵
  索引行**（不属任何挂载，只有全量重建的整表替换会抹掉它）。覆盖改排序/关开关/挪路径/换根目录/
  停用/删除/新增七条分支，全过。
- 真机实测（`webvid.exe` 5399 端口）：新增→只扫新盘；换根目录→2→5 条；改排序·关开关·挪路径→
  `scanning:false` 且条数不变（挪路径后搜索命中新前缀）；重载→只扫这一个盘；删除→5→1 条。
  全程 `index/progress.running` 恒 false，日志无一次「索引完成」= 没有全量重建。
- **下一步**：发版（版本号 + 更新日志 + `gh release edit --notes-file`）。
