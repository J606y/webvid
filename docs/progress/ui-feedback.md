...
## 封面与源信息：四段整改（2026-07-29）

用户诉求：挂载 → 索引 → 自动预载；进度条一进去就看得见；别把机器榨干；跑完每个视频都有封面。

**段1 · 把 ffmpeg/ffprobe 关进笼子**
- 所有 ffmpeg/ffprobe 加 `-threads 1`。抽一帧、读元数据本不需要多核，默认却抢满全部核心——
  这是「新建索引时 CPU 跑满」的直接原因。
- 新增 `util.Gate`（容量可热调的并发闸，带单测）。**探测原先完全没有闸**，缓存全空时
  来多少请求就起多少个 ffprobe；现在抽封面与探测共用一个总闸，播放转码不受限（那是前台活）。
- 预载并发从写死 4 改为可配。后台「任务设置 → 封面与源信息」两项：**预载并发**、**占用核数**，
  存 `preload_workers` / `media_jobs`，保存即热生效。

**段2 · 封面一次做成就永久留下（治「每次打开主页都重来」）**
- 真因：`thumbHandler` 把浏览器请求的 ctx 直接传给 ffmpeg。滚动/切页/懒加载一取消，
  ffmpeg 当场被杀、半成品丢弃、什么都不缓存 → 下次进主页从头再烧一遍 CPU 与带宽，永不收敛。
  日志里那句 `context canceled` 长期被当成「良性噪音」，其实是这个 bug 的指纹。
- 改：thumb / video-info 两个 handler 用 `context.WithoutCancel` + 超时（5min / 90s），
  生成过程与请求生死解绑；请求可以走，活照做完落盘。
- 实测：`curl --max-time 0.03` 客户端 30ms 强制断开（退出码 28），1 秒内封面照样落盘。

**段3 · 失败要重试，直到真的每个视频都有封面**
- `finish` 里有没做成的就排重试：2 分钟起，每轮翻倍，`AutoEvery`(6h) 封顶；全做成则退避清零。
  最常见的一幕是进程刚起、云盘驱动还没连上，整轮秒败——不重试就永远只能靠浏览时当场生成。
- `index.OnChange` 新钩子 + `preload.Schedule()`：日常增删改（上传、复制、新挂载扫完）也补跑一轮。
  防抖 30s（一次目录复制会连着通知上千次），且**绝不打断在跑的一轮**（大库那轮打断了永远跑不完）。
- 卡片显示「N 项没能预载：<原因>。稍后会自动重试。」

**段4 · 数字自解释**
- `cover_total` / `probe_total` 分母：卡片写「封面 24 / 42 · 视频源信息 7 / 7 已缓存」。
  用户反馈「预载信息和索引对不上」——51602 是索引全部条目（含目录与非媒体），
  而源信息只统计**非 mp4/m4v/mov/webm** 的视频（其余浏览器直接能播、无需探测）。

验证：`go test ./...` 全过（新增 gate 3 项 + preload 6 项）；
`index-loading-check` 13/13、`index-clear-check` 12/12、`preload-snooze-check` 11/11、
`upload-conflict-check` 12/12。
